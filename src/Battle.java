...
public class Battle {
	...
	public void battle(int[] monsterIds) {
		...
		while (battleLoop == 1) {
			...
			int mainChoice = 0;
			try {
				mainChoice = Integer.parseInt(io.getInput());
			} catch (Exception e) {
			}
	...
	public void attack() {
		...
		int atkChoice = 0;
		try {
			atkChoice = Integer.parseInt(io.getInput());
		} catch (Exception e) {
		}
	...
	public void run() {
		...
		runChance += ((currentEnergy * 10) / maxEnergy) / 2; // (1000/100) / 2 =
																// 5
		runChance -= ((((maxHealth - currentHealth) * 10) / maxHealth) / 4); // (1000/100)
																				// /
																				// 4
																				// =
																				// 2
